...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  if(rotateEl){
    const words=["In West Africa.","In Commerce.","In Real Estate.","In Digital Talent."];
    let idx=0;
  ...
  if (commImgs.length > 0) {
    ...
    setInterval(() => {
      if (window.innerWidth <= 640) {
        const lastIdx = commIdx;
        commIdx = (commIdx + 1) % commImgs.length;
        
        // Outgoing slide becomes last-active (retains opacity 1 at z-index 2)
        commImgs[lastIdx].classList.remove('active');
        commImgs[lastIdx].classList.add('last-active');
        
        // Incoming slide becomes active (fades in on top at z-index 3)
        commImgs[commIdx].classList.add('active');
        
        // After transition (800ms matching CSS), remove last-active from old slide
        setTimeout(() => {
          commImgs[lastIdx].classList.remove('last-active');
        }, 800);
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (rotator && words.length > 0) {
    let current = 0;
    setInterval(() => {
      current = (current + 1) % (words.length - 1); // -1 because last span is duplicate
